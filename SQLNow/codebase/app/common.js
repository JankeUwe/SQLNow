...
var A = {
    ...
    modules: {
        ...
        app: ["init", "sqlserver", "datenbanken", "verteilung", "customer", "maschine", "stammdaten"],
        common: ["settings_forms"]
       // scheduler: ["dhtmlxscheduler"],
       // schedulerext: ["dhtmlxscheduler_readonly", "dhtmlxscheduler_agenda_view"]
    }
};
...
function doOnLoad() {
    ...
    for (var a in A.modules) {
        for (var q = 0; q < A.modules[a].length; q++) {
            ...
            if (a == "scheduler") k.push("codebase/dhtmlx/scheduler/" + A.modules[a][q] + ".js");
            if (a == "schedulerext") k.push("codebase/dhtmlx/scheduler/ext/" + A.modules[a][q] + ".js");
        }
    }
    A.modules = k;
    // start loading
